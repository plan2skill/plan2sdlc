...
import type { EntityType, EntityChanges, EntityRecord } from '../types.js';
...
export function registryUpdate(
  ...
  if (existing.length > 0) {
    ...
    execute(
      'UPDATE entity SET is_current = 0, valid_to = ? WHERE id = ?',
      [now, old.id],
    );
    ...
    execute(
      `INSERT INTO entity (entity_type, name, path, purpose, domain, dependencies, decisions, last_task_id, valid_from, is_current)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
      [entityType, name, merged.path, merged.purpose, merged.domain, merged.dependencies, merged.decisions, taskId, now],
    );
  } else {
    // New entity
    execute(
