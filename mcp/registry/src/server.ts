...
function isRegistryEnabled(): boolean {
  ...
  if (!existsSync(configPath)) return true; // enabled by default when no config
  try {
...
// ---------------------------------------------------------------------------
// MCP protocol types (minimal, stdio JSON-RPC)
// ---------------------------------------------------------------------------
...
function handleToolCall(name: string, args: Record<string, unknown>): unknown {
  switch (name) {
    case 'registry_lookup':
      return registryLookup(args.entity_type as string as any, args.name as string);
    case 'registry_search':
      return registrySearch(args.query as string, args.entity_type as any, (args.limit as number) ?? 20);
    case 'registry_update':
      return registryUpdate(
        args.entity_type as any,
        args.name as string,
        args.changes as any,
        args.task_id as string,
      );
    case 'registry_history':
      return registryHistory(args.entity_type as any, args.name as string);
    case 'registry_domain_summary':
      return registryDomainSummary(args.domain as string);
    case 'registry_init_scan':
      return registryInitScan(args.project_dir as string, (args.domains as any[]) ?? []);
    default:
  ...
  process.stdin.on('data', (chunk: string) => {
    ...
    while (true) {
      ...
      try {
        ...
        if (res.id !== undefined) respond(res);
      } catch (err: any) {
