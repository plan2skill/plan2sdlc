...
async function detectPackageManager(projectDir) {
    if (await fileExists(join(projectDir, 'pnpm-lock.yaml')))
        return 'pnpm';
    if (await fileExists(join(projectDir, 'yarn.lock')))
        return 'yarn';
    if (await fileExists(join(projectDir, 'package-lock.json')))
        return 'npm';
    if ((await fileExists(join(projectDir, 'Pipfile'))) ||
        (await fileExists(join(projectDir, 'requirements.txt'))))
        return 'pip';
    if (await fileExists(join(projectDir, 'Gemfile')))
        return 'gem';
...
function detectLanguages(projectDir, deps, pm) {
    void projectDir; // unused but kept for future directory scanning
    const langs = [];
    ...
    if (pm === 'npm' || pm === 'yarn' || pm === 'pnpm') {
        if (!langs.includes('typescript'))
            ...
            langs.push('javascript'); // TS projects also have JS
    }
    if (pm === 'pip')
        langs.push('python');
    if (pm === 'gem')
...
function detectFrameworks(deps, requirementsTxt, gemfile) {
    const frameworks = [];
    const checks = [
        ['@nestjs/core', 'nestjs'],
        ['next', 'nextjs'],
...
async function detectOrms(projectDir, deps, frameworks) {
    const orms = [];
    if (hasDep(deps, 'prisma') ||
    ...
    if (hasDep(deps, 'typeorm'))
        orms.push('typeorm');
    if (hasDep(deps, 'sequelize'))
        orms.push('sequelize');
    if (hasDep(deps, 'mongoose'))
        orms.push('mongoose');
    if (hasDep(deps, 'drizzle-orm'))
        orms.push('drizzle');
    if (frameworks.includes('django'))
        orms.push('django-orm');
    return orms;
}
async function detectDatabases(projectDir) {
    const dbs = [];
    ...
    if (/postgres(ql)?:\/\//i.test(combined) || /POSTGRES/i.test(combined))
        dbs.push('postgresql');
    if (/mysql:\/\//i.test(combined))
        dbs.push('mysql');
    if (/mongodb:\/\/|MONGODB_URI/i.test(combined))
        dbs.push('mongodb');
    if (/file:|\.sqlite/i.test(combined))
        dbs.push('sqlite');
    if (/redis:\/\/|REDIS_URL/i.test(combined))
        dbs.push('redis');
    return dbs;
}
async function detectCicd(projectDir) {
    const cicd = [];
    if (await dirExists(join(projectDir, '.github', 'workflows')))
        cicd.push('github-actions');
    if (await fileExists(join(projectDir, '.gitlab-ci.yml')))
        cicd.push('gitlab-ci');
    if (await fileExists(join(projectDir, 'Jenkinsfile')))
        cicd.push('jenkins');
    if (await dirExists(join(projectDir, '.circleci')))
        cicd.push('circleci');
    if (await fileExists(join(projectDir, 'vercel.json')))
        cicd.push('vercel');
    return cicd;
}
async function detectMonorepoTools(projectDir) {
    const tools = [];
    if (await fileExists(join(projectDir, 'pnpm-workspace.yaml')))
        tools.push('pnpm-workspaces');
    if (await fileExists(join(projectDir, 'turbo.json')))
        tools.push('turborepo');
    if (await fileExists(join(projectDir, 'nx.json')))
        tools.push('nx');
    if (await fileExists(join(projectDir, 'lerna.json')))
        tools.push('lerna');
    return tools;
}
...
function detectTestFrameworks(deps) {
    ...
    if (hasDep(deps, 'jest') || hasDep(deps, '@jest/core') || hasDep(deps, 'ts-jest'))
        tests.push('jest');
    if (hasDep(deps, 'playwright') || hasDep(deps, '@playwright/test'))
        tests.push('playwright');
    if (hasDep(deps, 'cypress'))
        tests.push('cypress');
    if (hasDep(deps, 'pytest') || hasDep(deps, 'pytest-cov'))
        tests.push('pytest');
